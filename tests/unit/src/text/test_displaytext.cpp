...
namespace { // test namespace
	...
	TEST_CASE("DisplayText class", "[text][DisplayText]")
	{
		SECTION("Class Traits")
		{
			...
			CHECK(std::is_standard_layout_v<T> == std::is_standard_layout_v<std::string>);
			...
			CHECK(std::is_trivially_destructible_v<T> == std::is_trivially_destructible_v<std::string>);
		}
		SECTION("Construction Traits")
		{
			...
			CHECK(
				std::is_nothrow_copy_constructible_v<T> == std::is_nothrow_copy_constructible_v<std::string>);
		...
		SECTION("Copy Traits")
		{
			...
			CHECK(std::is_trivially_copyable_v<T> == std::is_trivially_copyable_v<std::string>);
			CHECK_FALSE(std::is_trivially_copy_assignable_v<T>);
			CHECK(std::is_trivially_copy_assignable_v<T> == std::is_trivially_copy_assignable_v<std::string>);
			CHECK_FALSE(std::is_nothrow_copy_assignable_v<T>);
			CHECK(std::is_nothrow_copy_assignable_v<T> == std::is_nothrow_copy_assignable_v<std::string>);
		}
		SECTION("Move Traits")
		{
			CHECK(std::is_move_assignable_v<T>);
			CHECK_FALSE(std::is_trivially_move_assignable_v<T>);
			CHECK(std::is_trivially_move_assignable_v<T> == std::is_trivially_move_assignable_v<std::string>);
			// In gcc-5 (Steam Scout Runtime), move-assigning a string may throw
			CHECK(std::is_nothrow_move_assignable_v<T> == std::is_nothrow_move_assignable_v<std::string>);
		}
	}
