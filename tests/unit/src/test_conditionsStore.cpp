...
namespace { // test namespace
	...
	std::string verifyAndStripPrefix(const std::string &prefix, const std::string &inputString)
	{
		if(inputString.size() < prefix.size() || (0 != inputString.compare(0, prefix.size(), prefix)))
			throw std::runtime_error(
				"String \"" + inputString + "\" does not start with prefix \"" + prefix + "\"");
	...
	public:
		void SetROPrefixProvider(ConditionsStore &store, const std::string &prefix)
		{
			...
			conditionsProvider.SetHasFunction(
				[this, prefix](const std::string &name)
				{
					verifyAndStripPrefix(prefix, name);
					return isInMap(values, name);
				});
			conditionsProvider.SetSetFunction(
				[prefix](const std::string &name, int64_t value)
				{
					verifyAndStripPrefix(prefix, name);
					return false;
				});
			conditionsProvider.SetEraseFunction(
				[prefix](const std::string &name)
				{
					verifyAndStripPrefix(prefix, name);
					return false;
				});
			conditionsProvider.SetGetFunction(
				[this, prefix](const std::string &name)
				{
					verifyAndStripPrefix(prefix, name);
		...
		void SetRWPrefixProvider(ConditionsStore &store, const std::string &prefix)
		{
			...
			conditionsProvider.SetSetFunction(
				[this, prefix](const std::string &name, int64_t value)
				{
					verifyAndStripPrefix(prefix, name);
					values[name] = value;
					return true;
		...
		void SetRONamedProvider(ConditionsStore &store, const std::string &named)
		{
			...
			conditionsProvider.SetHasFunction(
				[this, named](const std::string &name)
				{
					verifyName(named, name);
					return isInMap(values, name);
				});
			conditionsProvider.SetSetFunction(
				[named](const std::string &name, int64_t value)
				{
					verifyName(named, name);
					return false;
				});
		...
		void SetRWNamedProvider(ConditionsStore &store, const std::string &named)
		{
			...
			conditionsProvider.SetSetFunction(
				[this, named](const std::string &name, int64_t value)
				{
					verifyName(named, name);
					values[name] = value;
					return true;
	...
	SCENARIO("Creating a ConditionsStore", "[ConditionsStore][Creation]")
	{
		GIVEN("A ConditionStore")
		{
			...
			{
				const auto store =
					ConditionsStore{{"a", 1}, {"b", 2}, {"d", 4}, {"c", 3}, {"g", 7}, {"f", 6}, {"e", 5}};
