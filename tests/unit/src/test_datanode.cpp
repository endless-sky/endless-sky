...
namespace { // test namespace
	...
	TEST_CASE("DataNode Basics", "[DataNode]")
	{
		...
		SECTION("Class Traits")
		{
			...
			// The class layout apparently satisfies StandardLayoutType when building/testing for Steam, but
			// false otherwise. This may change in the future, with the expectation of false everywhere (due
			// to the list<DataNode> field). CHECK_FALSE( std::is_standard_layout_v<T> );
	...
	SCENARIO("Determining if a token is numeric", "[IsNumber][Parsing][DataNode]")
	{
		GIVEN("An integer string")
		{
			...
			{
				auto strNum = GENERATE(as<std::string>{}, "1", "10", "100", "1000000000000000");
