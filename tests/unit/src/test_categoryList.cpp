...
namespace { // test namespace
	...
	SCENARIO("Creating a CategoryList", "[CategoryList][Creation]")
	{
		GIVEN("a category list")
		{
			...
			auto equal = [](const std::string &s, const CategoryList::Category &c) noexcept -> bool
			{
				return s == c.Name();
			};
			...
			WHEN("sorted contents are given to the list without precedence")
			{
				...
				sorted.push_back("first"); // Precedence = 0, as that is the default precedence of the first
										   // added cateogry.
				sorted.push_back(
					"second"); // Precedence = 1, as each new category uses the last used precedence + 1.
			...
			WHEN("unsorted contents are given to the list with precedence")
			{
				...
				THEN("sorting the list correctly changes its ordering")
				{
					...
					AND_WHEN("a list is loaded again with two categories of the same precedence")
					{
						...
						THEN("after sorting, the categories with the same precedence become alphabetically "
							 "ordered")
						{
					...
					AND_WHEN("a list is loaded again with a category that already exists but with a "
