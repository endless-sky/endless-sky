...
namespace Catch {
	...
	struct StringMaker<EsUuid> {
		static std::string convert(const EsUuid &value) { return value.ToString(); }
	};
}
...
namespace { // test namespace
	...
	struct InstantiableContainer : public Identifiable {
		...
		{
			auto result = std::vector<std::string>{id.ToString()};
			for(auto &&ic : items)
			{
	...
	TEST_CASE("EsUuid class", "[uuid]")
	{
		...
		SECTION("Construction Traits")
		{
			...
			// Ensuring the memory associated with the UUID is initialized means a non-trivial default
			// constructor.
	...
	SCENARIO("Creating a UUID", "[uuid][creation]")
	{
		...
		GIVEN("a reference string")
		{
			...
			WHEN("the string is invalid")
			{
				...
				{
					auto invalid = GENERATE(as<std::string>{}, "abcdef",
						"ZZZZZZZZ-ZZZZ-ZZZZ-ZZZZ-ZZZZZZZZZZZZ", "5be91256-f6ba-47cd-96df-1ce1cb-fee86");
	...
	SCENARIO("Copying uniquely identifiable objects", "[uuid][copying]")
	{
		...
		// can be identified as a starting ship at a later instance. The same goes for ships gifted by
		// missions: a later mission should be able to identify which of the player's ships was gifted by some
		// particular previous mission.)
	...
	{
		auto MakeContainer =
			[](const std::string &parentId, const std::string &childId, const std::string &otherId)
		{
			InstantiableContainer result;
