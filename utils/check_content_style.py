...
# 'permanent_errors' and 'permanent_warnings' are preserved after reloading the data, useful when a fix results in a warning
...
class CheckResult(object):
	def __init__(self, errors=None, warnings=None, fixed_errors=None, fixed_warnings=None, permanent_errors=None, permanent_warnings=None, new_file_contents=None):
		self.fixed_errors = fixed_errors
		self.fixed_warnings = fixed_warnings
		self.errors = errors
		self.warnings = warnings
		self.permanent_errors = permanent_errors
		self.permanent_warnings = permanent_warnings
		...
		if permanent_warnings is None:
			self.permanent_warnings = []
	...
	# Has no return value.
	def combine_with(self, result) -> None:
		self.fixed_errors += result.fixed_errors
		self.fixed_warnings += result.fixed_warnings
		self.errors += result.errors
		self.warnings += result.warnings
		self.permanent_errors += result.permanent_errors
		self.permanent_warnings += result.permanent_warnings
...
# Aliasing the type of dict generated by parse_line()
TokenDict = dict[TokenType, str | tuple[str, QuoteType] | list[tuple[str, QuoteType]]]
...
def check_line_separators(contents: list[str], auto_correct: bool, config: dict[str]) -> CheckResult:
	result = CheckResult()
...
# config: the script configuration
# Return value: a CheckResult
def check_copyright(contents: list[str], auto_correct: bool, config: dict[str]) -> CheckResult:
...
# Counts the level of indentation.
# Parameters:
# indent: one level of the type of indentation used
# text: the indentation at the beginning of the text
# Returns: the level of indentation
def count_indent(indent: str, text: str) -> int:
	return int(len(text) / len(indent))


# Checks the indentation of each line in the file.
# Parameters:
# contents: the contents of the file
# parsed_lines: the contents of the file, parsed into tokens
# auto_correct: whether to attempt to correct the issue
# config: the script configuration
# Return value: a CheckResult
def check_indentation(contents: list[str], parsed_lines: list[TokenDict], auto_correct: bool, config: dict[str]) -> CheckResult:
	...
	def get_expected_indent(indent: str, index: int) -> int:
		for i in range(index + 1, len(contents)):
			if not (contents[i].isspace() or contents[i] == ""):
				return count_indent(indent, parsed_lines[i][TokenType.INDENT])
		return 0
	...
	for index, (line, parsed_line) in enumerate(zip(contents, parsed_lines)):
		level = count_indent(indent, parsed_line[TokenType.INDENT])
		if level - previous_level > max_delta or (level > previous_level and level > get_expected_indent(indent, index - 2) and level > get_expected_indent(indent, index) and line.isspace()):
		...
		if line.isspace() or line == "":
			...
			if error_if_indent:
				...
				expected = get_expected_indent(indent, index)
				if level != expected:
			...
			previous_level = count_indent(indent, parsed_line[TokenType.INDENT])
	return result
...
def parse_line(line: str) -> TokenDict:
	# Finds each parameter and their quotation types.
	def find_parameter_types(node: str, line: str) -> list[tuple[str, QuoteType]]:
		types = []
		current_separator = ""
		contents = []
	...
	def parse_node(line: str) -> tuple[str, QuoteType]:
		line = line.lstrip()
...
def generate_line(tokens: TokenDict) -> str:
...
# config: the script configuration
# Return value: a CheckResult
def check_child_node_order(contents: list[str], parsed_lines: list[TokenDict], auto_correct: bool, config: dict[str]) -> CheckResult:
	result = CheckResult()
	result.new_file_contents = [line for line in contents]
	...
	for index, (line, parsed_line) in enumerate(zip(contents, parsed_lines)):
		...
		# Finding parent node
		indent = count_indent(indentation, parsed_line[TokenType.INDENT])
		node = parsed_line[TokenType.NODE][0]
...
# config: the script configuration
# Return value: a CheckResult
def check_node_parameters(contents: list[str], parsed_lines: list[TokenDict], auto_correct: bool, config: dict[str]) -> CheckResult:
	result = CheckResult()
	result.new_file_contents = [line for line in contents]

	# Replaces the nth parameter with a differently quoted parameter of the same value.
	def find_quote_type(text: str, allowed_types: list[QuoteType], index: int) -> QuoteType:
		...
		if QuoteType.NONE in allowed_types:
			...
			types = " or ".join([type.value for type in original_types])
			result.permanent_errors.append(Error(index, f"Parameter cannot be quoted properly: expected {types}, but none are valid"))
	...
	for index, tokens in enumerate(parsed_lines):
						...
						for (paramIndex, (expected, actual)) in enumerate(zip(expected_types, types)):
							...
							if actual not in expected and len(expected) > 0:
								...
								if auto_correct:
									...
									tokens[TokenType.PARAM][paramIndex] = (tokens[TokenType.PARAM][paramIndex][0], find_quote_type(tokens[TokenType.PARAM][paramIndex][0], expected_types[paramIndex], index))
...
# config: the script configuration
# Return value: a CheckResult
def check_with_regex(check_group: dict[str, list], contents: list[str], auto_correct: bool, config: dict[str]) -> CheckResult:
	result = CheckResult()
	result.new_file_contents = [line for line in contents]
...
def find_text_lines(contents: list[str], parsed_lines: list[TokenDict], config: dict[str], excluded_nodes: list[str], exclude_comments: bool, exclude_keywords: bool) -> list[str]:
	new_contents = []
	...
	for parsed_line in parsed_lines:
		new_line = dict(parsed_line)
		if exclude_comments:
			new_line[TokenType.COMMENT] = ""
		if len(new_line[TokenType.NODE][0]) == 0:
			new_contents.append(None)
			# Comment or empty line
			continue
		if is_word:
			if count_indent(indent, new_line[TokenType.INDENT]) <= word_indent_level:
				is_word = False
		line = generate_line(new_line)
		if not is_word:
			stripped = line.strip()
			for node in excluded_nodes:
				if re.search(node, stripped) is not None:
					is_word = True
					word_indent_level = count_indent(indent, new_line[TokenType.INDENT])
					break
...
# config: the script configuration rules.
def rewrite(file: str, contents: list[str], config: dict[str]) -> None:
...
# Return value: a CheckResult
def check_content_style(file: str, auto_correct: bool, config: dict[str]) -> CheckResult:
	issues = CheckResult()
	fixed_errors = []
	fixed_warnings = []
	...
	while True:
		...
		issues.combine_with(check_line_separators(contents, auto_correct, config))
		...
		parsed_lines = [parse_line(line) for line in contents]

		# Checking indentation
		issues.combine_with(check_indentation(contents, parsed_lines, auto_correct, config))
		if issues.should_reload():
			do_reload()
			continue

		# Checking the order of child nodes everywhere
		issues.combine_with(check_child_node_order(contents, parsed_lines, auto_correct, config))
		if issues.should_reload():
			do_reload()
			continue

		# Checking the quotes for node parameters
		issues.combine_with(check_node_parameters(contents, parsed_lines, auto_correct, config))
		if issues.should_reload():
			do_reload()
			continue
		...
		for check_group in config["regexChecks"]:
			...
			restricted_contents = find_text_lines(contents, parsed_lines, config, excluded_nodes, exclude_comments, exclude_keywords)
...
def print_file_result(file: str, result: CheckResult) -> None:
...
# No return value. The formatted message is written to the output.
def print_result(fixed_errors: int, fixed_warnings: int, errors: int, warnings: int) -> None:
	text = ""
...
if __name__ == '__main__':
	...
	for file in data_files:
		...
		# Merge permanent errors into the errors from the last iteration
		result.errors += result.permanent_errors
		result.warnings += result.permanent_warnings
