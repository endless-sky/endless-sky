...
if __name__ == '__main__':
	...
	if len(sys.argv[1:]) > 0:
		...
		files = glob.glob('source/**/*.cpp', recursive=True) + glob.glob('source/**/*.h', recursive=True) + glob.glob('tests/**/!(catch_amalgamated)*.cpp', recursive=True) + glob.glob('tests/**/*.h', recursive=True)
	files.sort()
