...
int64_t ShopPanel::LicenseCost(const Ship *ship, bool onlyStoreModels) const
{
	// If the player is repurchasing a ship he just sold to the store, no license is needed.
	if(!onlyStoreModels && player.Stock(ship) > 0)
		return 0;

	// Otherwise, retrieve license information from the attributes.
	return LicenseCost(&ship->Attributes());
}
