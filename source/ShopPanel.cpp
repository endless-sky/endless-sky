...
namespace {
	...
	const Color selectedColor(.8f, 1.f);
	const Color unselectedColor(.4f, 1.f);
...
bool ShopPanel::Release(int x, int y)
{
	...
	const auto &dropZone = find_if(shipZones.begin(), shipZones.end(),
		[&](const ClickZone<const Ship *> &zone){ return zone.Contains(this->dragPoint); });

	if(dropZone == shipZones.end() || dropZone->Contains(dragStart))
		return true;

	const Ship *dropShip = dropZone->Value();
	...
	if(ship->get() == dropShip && (dragStart.Y() < dropZone->Top() || (dragStart.Y() <= dropZone->Bottom()
			&& dragStart.X() < dropZone->Left())))
	{
		if(!shipSelection.Has(ship->get()))
			newOrder.push_back(*ship);
		++ship;
...
void ShopPanel::DrawShipsSidebar()
{
	...
	for(const shared_ptr<Ship> &ship : player.Ships())
	{
		...
		const bool isHovered = shipZones.back().Contains(mouse);
...
void ShopPanel::DrawPlayerShip(const Ship &ship, const Point &point, const bool isSelected) const
{
	...
	if(sprite)
	{
		...
		if(Preferences::Has(SHIP_OUTLINES))
		{
			...
			OutlineShader::Draw(sprite, point, size, isSelected ? selectedColor : unselectedColor);
		}
		else
		{
