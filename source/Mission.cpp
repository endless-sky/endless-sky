...
Mission Mission::Instantiate(const PlayerInfo &player, const shared_ptr<Ship> &boardingShip) const
{
	...
	for(const NPC &npc : npcs)
		result.npcs.push_back(npc.Instantiate(player.Conditions(), subs, sourceSystem, result.destination->GetSystem(),
			jumps, payload));
	...
	for(const auto &it : actions)
		result.actions[it.first] = it.second.Instantiate(
	...
	for(const auto &it : onEnter)
		result.onEnter[it.first] = it.second.Instantiate(
			player.Conditions(), subs, sourceSystem, jumps, payload);
