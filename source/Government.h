...
class Government {
  public:
    ...
    // Get the amount that your reputation changes for the given offense against
    // the given government. The given value should be a combination of one or
    // more ShipEvent values. Returns 0 if the Government is null.
    ...
    // Find out if this government should send custom hails even if the player
    // does not know its language.
    ...
    // The second attribute denotes the minimal and maximal attraction required
    // for the fleet to appear.
    const std::vector<RaidFleet> &RaidFleets() const;

    // Check if, according to the politics stored by GameData, this government
    // is an enemy of the given government right now.
    ...
    // Politics object, but are provided as member functions here for clearer
    // syntax.
    ...
    std::string Fine(PlayerInfo &player, int scan = 0,
    ...
    // Determine if ships from this government can travel to the given system or
    // planet.
  ...
  private:
    ...
    // If a government appears in this set, and the reputation with this
    // government is affected by actions, and events performed against that
    // government, use the penalties that government applies for the action
    // instead of this government's own penalties.
