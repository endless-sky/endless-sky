...
void MainPanel::Draw()
{
	...
	if(isDragging)
	{
		if(canDrag)
		{
			...
			LineShader::Draw(dragSource, Point(dragSource.X(), dragPoint.Y()), .4f, dragColor, false);
			LineShader::Draw(Point(dragSource.X(), dragPoint.Y()), dragPoint, .4f, dragColor, false);
			LineShader::Draw(dragPoint, Point(dragPoint.X(), dragSource.Y()), .4f, dragColor, false);
			LineShader::Draw(Point(dragPoint.X(), dragSource.Y()), dragSource, .4f, dragColor, false);
		}
		else
			isDragging = false;
	}
