...
int Ship::TakeDamage(const Projectile &projectile, bool isBlast)
{
	...
	double shieldDamage = weapon.ShieldDamage() * damageScaling / (1 + max(-0.99, attributes.Get("shield protection")));
	double hullDamage = weapon.HullDamage() * damageScaling / (1 + max(-0.99, attributes.Get("hull protection")));
	double hitForce = weapon.HitForce() * damageScaling / (1 + max(-0.99, attributes.Get("force protection")));
	double fuelDamage = weapon.FuelDamage() * damageScaling / (1 + max(-0.99, attributes.Get("fuel protection")));
	double heatDamage = weapon.HeatDamage() * damageScaling / (1 + max(-0.99, attributes.Get("heat protection")));
	double ionDamage = weapon.IonDamage() * damageScaling / (1 + max(-0.99, attributes.Get("ion protection")));
	double disruptionDamage = weapon.DisruptionDamage() * damageScaling / (1 + max(-0.99, attributes.Get("disruption protection")));
	double slowingDamage = weapon.SlowingDamage() * damageScaling / (1 + max(-0.99, attributes.Get("slowing protection")));
	bool wasDisabled = IsDisabled();
	bool wasDestroyed = IsDestroyed();
	
	double shieldFraction = 1. - weapon.Piercing() / (1 + max(-0.99, attributes.Get("piercing protection")));
