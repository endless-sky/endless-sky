...
const vector<Angle> BAY_ANGLE = {Angle(0.), Angle(-90.), Angle(90.),
                                 Angle(180.)};
...
void DoRepair(double &stat, double &available, double maximum, double &energy,
              double energyCost, double &fuel, double fuelCost) {
...
const vector<string> Ship::CATEGORIES = {
    ...
    "Interceptor",   "Light Warship",   "Medium Warship",
    "Heavy Warship", "Fighter",         "Drone"};

// Construct and Load() at the same time.
Ship::Ship(const DataNode &node) { Load(node); }
...
void Ship::Load(const DataNode &node) {
  ...
  for (const DataNode &child : node) {
    ...
    if (add && (child.Size() < 2 || child.Token(1) != "attributes")) {
      child.PrintTrace(
          "Skipping invalid use of 'add' with " +
    ...
    if (key == "sprite")
      ...
      // Print a warning for the first hardpoint after 32, i.e. only 1 warning
      // per ship. if(armament.Get().size() == 33)
      // child.PrintTrace("Warning: ship has more than 32 weapon hardpoints.
      // Some weapons may not fire:");
...
void Ship::FinishLoading(bool isNewInstance) {
  ...
  for (auto &it : equipped) {
    ...
    if (excess > 0) {
      ...
      if (!name.empty())
        warning += " \"" + name + "\"";
      warning += ": outfit \"" + it.first->Name() +
                 "\" equipped but not included in outfit list.";
      ...
      if (!name.empty())
        warning += " \"" + name + "\"";
      warning += ": outfit \"" + it.first->Name() +
                 "\" is not a weapon, but is installed as one.";
      Files::LogError(warning);
    }
  }
  ...
  for (const Hardpoint &hardpoint : armament.Get()) {
    ...
    if (outfit &&
      ...
      warning += (hardpoint.IsTurret() ? "turret but is a gun.\n\tturret"
                                       : "gun but is a turret.\n\tgun");
      warning += to_string(2. * hardpoint.GetPoint().X()) + " " +
                 to_string(2. * hardpoint.GetPoint().Y());
      warning += " \"" + outfit->Name() + "\"";
      Files::LogError(warning);
    }
  }
  ...
  // Add a default "launch effect" to any internal bays if this ship is crewed
  // (i.e. pressurized).
  for (Bay &bay : bays)
  ...
  // Issue warnings if this ship has negative outfit, cargo, weapon, or engine
  // capacity.
  string warning;
  for (const string &attr : set<string>{"outfit space", "cargo space",
  ...
  if (!warning.empty()) {
    ...
    // no names. Print the outfits to facilitate identifying this ship
    // definition.
    string message = (!name.empty() ? "Ship \"" + name + "\" " : "") + "(" +
                     modelName + "):\n";
  ...
  out.BeginChild();
  {
    ...
    for (const Hardpoint &hardpoint : armament.Get()) {
      ...
      if (hardpoint.GetOutfit())
        out.Write(type, 2. * hardpoint.GetPoint().X(),
                  2. * hardpoint.GetPoint().Y(), hardpoint.GetOutfit()->Name());
      else
        out.Write(type, 2. * hardpoint.GetPoint().X(),
...
const string &Ship::Name() const { return name; }

const string &Ship::ModelName() const { return modelName; }

const string &Ship::PluralModelName() const { return pluralModelName; }
...
const string &Ship::Description() const { return description; }
...
string Ship::FlightCheck() const {
  double generation = attributes.Get("energy generation") -
  ...
  double fuelChange =
      attributes.Get("fuel generation") - attributes.Get("fuel consumption");
...
void Ship::SetSystem(const System *system) { currentSystem = system; }
...
void Ship::SetHail(const Phrase &phrase) { hail = &phrase; }
...
string Ship::GetHail(const PlayerInfo &player) const {
  ...
  string hailStr =
      hail ? hail->Get() : government ? government->GetHail(isDisabled) : "";
...
void Ship::Move(vector<Visual> &visuals, list<shared_ptr<Flotsam>> &flotsam) {
  ...
  if (IsDestroyed()) {
      ...
      if (!forget) {
        ...
        for (int i = 0; i < debrisCount; ++i) {
          ...
          Point effectVelocity =
              velocity + angle.Unit() * (scale * Random::Real());
    ...
    for (const Leak &leak : leaks)
      if (leak.openPeriod > 0 && !Random::Int(leak.openPeriod)) {
        ...
        activeLeaks.back().angle =
            Angle(outline[i] - outline[i + 1]) + Angle(90.);
      }
      ...
      if (leak.effect) {
        ...
        if (Random::Int(2))
          visuals.emplace_back(*leak.effect,
                               angle.Rotate(leak.location) + position, velocity,
                               leak.angle + angle);
    ...
    if (isUsingJumpDrive && !forget)
      CreateSparks(visuals, "jump drive",
                   hyperspaceCount * Width() * Height() * .000006);
    ...
    if (hyperspaceCount == HYPER_C) {
      ...
      const Planet *planet =
          (targetPlanet ? targetPlanet->GetPlanet() : nullptr);
      ...
      if (!targetPlanet && parent && parent->targetPlanet) {
        ...
        if (planet && !planet->IsWormhole() &&
            planet->IsInSystem(currentSystem))
      ...
      if (isUsingJumpDrive) {
        position =
            target + Angle::Random().Unit() * 300. * (Random::Real() + 1.);
        return;
      }
  ...
  if (isDisabled) {
    ...
    isUsingJumpDrive = !attributes.Get("hyperdrive") ||
                       !currentSystem->Links().count(hyperspaceSystem);
  ...
  if (pilotError)
    ...
    // If the ship is disabled, don't show a warning message due to missing
    // crew.
  } else if (requiredCrew &&
             static_cast<int>(Random::Int(requiredCrew)) >= Crew()) {
    pilotError = 30;
    if (parent.lock() || !isYours)
      Messages::Add("The " + name +
                    " is moving erratically because there are not enough crew "
                    "to pilot it.");
    else
      Messages::Add("Your ship is moving erratically because you do not have "
                    "enough crew to pilot it.");
  } else
    pilotOkay = 30;
  ...
  if (isDisabled)
    ...
    if (commands.Turn()) {
      ...
      if (energy < cost * fabs(commands.Turn()))
        commands.SetTurn(commands.Turn() * energy /
                         (cost * fabs(commands.Turn())));
    ...
    double thrustCommand =
        commands.Has(Command::FORWARD) - commands.Has(Command::BACK);
    double thrust = 0.;
    if (thrustCommand) {
      // Check if we are able to apply this thrust.
      double cost =
          attributes.Get((thrustCommand > 0.) ? "thrusting energy"
      ...
      if (thrustCommand) {
        ...
        if (thrust) {
          ...
          energy -= scale * cost;
          heat +=
              scale * attributes.Get(isThrusting ? "thrusting heat"
                                                 : "reverse thrusting heat");
          acceleration += angle.Unit() * (thrustCommand * thrust / mass);
        }
      }
    }
    bool applyAfterburner = (commands.Has(Command::AFTERBURNER) ||
                             (thrustCommand > 0. && !thrust)) &&
                            !CannotAct();
    if (applyAfterburner) {
        ...
        if (!forget)
          for (const EnginePoint &point : enginePoints) {
            ...
            for (const auto &it : attributes.AfterburnerEffects())
              for (int i = 0; i < it.second; ++i)
                visuals.emplace_back(*it.first, pos + velocity,
  ...
  if (acceleration) {
    ...
    Point dragAcceleration =
        acceleration - velocity * (attributes.Get("drag") / mass);
    // Make sure dragAcceleration has nonzero length, to avoid divide by zero.
    if (dragAcceleration) {
      // What direction will the net acceleration be if this drag is applied?
      // If the net acceleration will be opposite the thrust, do not apply drag.
      dragAcceleration *=
          .5 * (acceleration.Unit().Dot(dragAcceleration.Unit()) + 1.);
  ...
  if (target && !isDisabled) {
    ...
    if (isBoarding && !CanBeCarried()) {
      if (!target->IsDisabled() && government->IsEnemy(target->government))
        isBoarding = false;
      else if (target->IsDestroyed() || target->IsLanding() ||
               target->IsHyperspacing() || target->GetSystem() != GetSystem())
        isBoarding = false;
    }
    if (isBoarding && !pilotError) {
      ...
      if (distance < 10. && speed < 1. && (CanBeCarried() || !turn)) {
        if (cloak) {
          ...
          if (isEnemy &&
              Random::Real() < target->Attributes().Get("self destruct")) {
            Messages::Add("The " + target->ModelName() + " \"" +
                          target->Name() +
  ...
  if (target && target->IsDestroyed() &&
      target->explosionCount >= target->explosionTotal)
    targetShip.reset();
...
void Ship::DoGeneration() {
  ...
  if (!isDisabled) {
    ...
    const double shieldsEnergy =
        attributes.Get("shield energy") / shieldsAvailable;
    ...
    DoRepair(shields, shieldsRemaining, attributes.Get("shields"), energy,
             shieldsEnergy, fuel, shieldsFuel);
    ...
    if (!bays.empty()) {
      ...
      sort(
          ...
          (isYours && Preferences::Has(FIGHTER_REPAIR))
              // Players may use a parallel strategy, to launch fighters in
              // waves.
              ? [](const pair<double, Ship *> &lhs,
                   const pair<double, Ship *>
                       &rhs) { return lhs.first > rhs.first; }
              // The default strategy is to prioritize the healthiest ship
              // first, in order to get fighters back out into the battle as
              // soon as possible.
              : [](const pair<double, Ship *> &lhs,
                   const pair<double, Ship *> &rhs) {
      ...
      for (const pair<double, Ship *> &it : carried) {
        Ship &ship = *it.second;
        DoRepair(ship.hull, hullRemaining, ship.attributes.Get("hull"), energy,
                 hullEnergy, fuel, hullFuel);
        DoRepair(ship.shields, shieldsRemaining, ship.attributes.Get("shields"),
                 energy, shieldsEnergy, fuel, shieldsFuel);
      }

      // Now that there is no more need to use energy for hull and shield
      // repair, if there is still excess energy, transfer it.
      double energyRemaining =
          min(0., energy - attributes.Get("energy capacity"));
      double fuelRemaining = min(0., fuel - attributes.Get("fuel capacity"));
      for (const pair<double, Ship *> &it : carried) {
        Ship &ship = *it.second;
        DoRepair(ship.energy, energyRemaining,
                 ship.attributes.Get("energy capacity"));
        DoRepair(ship.fuel, fuelRemaining,
  ...
  if (disruption)
    disruption =
        max(0., .99 * disruption - attributes.Get("disruption resistance"));
  if (slowness)
  ...
  isDisabled =
      isOverheated || hull < MinimumHull() || (!crew && RequiredCrew());
  ...
  if (isDisabled)
    ...
    // close to the star. Carried fighters can't collect fuel or energy this
    // way.
    if (currentSystem) {
      double scale = .2 + 1.8 / (.001 * position.Length() + 1);
      fuel += currentSystem->SolarWind() * .03 * scale *
              (sqrt(attributes.Get("ramscoop")) + .05 * scale);

      energy += currentSystem->SolarPower() * scale *
    ...
    // Convert fuel into energy and heat only when the required amount of fuel
    // is available.
...
void Ship::Launch(list<shared_ptr<Ship>> &ships, vector<Visual> &visuals) {
  ...
  // is landing, jumping, or cloaked. If already destroyed (e.g.
  // self-destructing), eject any ships still docked, possibly destroying them
  // in the process.
  bool ejecting = IsDestroyed();
  if (!ejecting && (!commands.Has(Command::DEPLOY) || zoom != 1.f ||
                    hyperspaceCount || cloak))
    return;

  for (Bay &bay : bays)
    if (bay.ship && ((bay.ship->Commands().Has(Command::DEPLOY) &&
                      !Random::Int(40 + 20 * bay.isFighter)) ||
      ...
      if (!ejecting) {
        ...
        if (maxFuel) {
          ...
          if (spareFuel > 0.)
            TransferFuel(min(maxFuel - bay.ship->fuel, spareFuel),
                         bay.ship.get());
      ...
      Angle launchAngle = ejecting ? Angle(exitPoint - position)
                                   : angle + BAY_ANGLE[bay.facing];
      Point v = velocity + (.3 * maxV) * launchAngle.Unit() +
                (.2 * maxV) * Angle::Random().Unit();
...
shared_ptr<Ship> Ship::Board(bool autoPlunder) {
  ...
  if (CannotAct() || !victim || victim->IsDestroyed() ||
...
int Ship::Scan() {
  ...
  int result = 0;
  auto doScan = [&](double &elapsed, const double speed,
                    const double scannerRange, const int event) -> void {
  ...
  if (startedScanning && isYours) {
    if (!target->Name().empty())
      Messages::Add("Attempting to scan the " + target->Noun() + " \"" +
                        target->Name() + "\".",
                    false);
    else
  ...
  for (unsigned i = 0; i < hardpoints.size(); ++i) {
    ...
    if (weapon && CanFire(weapon)) {
      if (weapon->AntiMissile())
        antiMissileRange =
            max(antiMissileRange, weapon->Velocity() + weaponRadius);
...
bool Ship::FireAntiMissile(const Projectile &projectile,
...
bool Ship::IsTargetable() const {
  return (zoom == 1.f && !explosionRate && !forget && !isInvisible &&
...
bool Ship::CanLand() const {
  if (!GetTargetStellar() || !GetTargetStellar()->GetPlanet() || isDisabled ||
      IsDestroyed())
    return false;
...
bool Ship::IsHyperspacing() const { return hyperspaceCount != 0; }
...
bool Ship::IsReadyToJump(bool waitingIsReady) const {
  // Ships can't jump while waiting for someone else, carried, or if already
  // jumping.
  ...
  bool isJump = !attributes.Get("hyperdrive") ||
                !currentSystem->Links().count(targetSystem);
...
int Ship::CustomSwizzle() const { return customSwizzle; }
...
void Ship::WasCaptured(const shared_ptr<Ship> &capturer) {
  ...
  if (transfer > 0) {
    if (totalRequired > capturer->Crew() + crew)
      transfer =
          max(crew ? 0 : 1, (capturer->Crew() * transfer) / totalRequired);
...
double Ship::Health() const {
  ...
  // Consider hull-only and pooled health, compensating for any reductions by
  // disruption damage.
  return min(spareHull / hullDivisor,
             (spareHull + shields / (1. + disruption * .01)) / divisor);
}
...
double Ship::JumpFuel(const System *destination) const {
  ...
  if (attributes.Get("jump drive") &&
      currentSystem->Neighbors().count(destination))
...
double Ship::JumpFuelMissing() const {
  ...
  double jumpFuel = JumpFuel(targetSystem);
  if (!jumpFuel || fuel > jumpFuel ||
      jumpFuel > attributes.Get("fuel capacity"))
    return 0.;
...
double Ship::TurnRate() const { return attributes.Get("turn") / Mass(); }
...
int Ship::TakeDamage(const Projectile &projectile, bool isBlast) {
  ...
  if (isBlast && weapon.IsDamageScaled()) {
    ...
    double d = max(0., (projectile.Position() - position).Length() -
                           GetMask().Radius());
    double rSquared = d * d / (blastRadius * blastRadius);
    damageScaling *=
        k / ((1. + rSquared * rSquared) * (1. + rSquared * rSquared));
  }
  ...
  if (!isBlast && projectile.GetGovernment() &&
      !projectile.GetGovernment()->IsEnemy(government) &&
...
bool Ship::Carry(const shared_ptr<Ship> &ship) {
  ...
  for (Bay &bay : bays)
    if ((bay.isFighter == isFighter) && !bay.ship) {
      ...
      // Return unused fuel to the carrier, for any launching fighter that needs
      // it.
...
const vector<Ship::Bay> &Ship::Bays() const { return bays; }
...
void Ship::Jettison(const string &commodity, int tons) {
  ...
  for (; tons > 0; tons -= Flotsam::TONS_PER_BOX)
    jettisoned.emplace_back(new Flotsam(
        commodity,
        (Flotsam::TONS_PER_BOX < tons) ? Flotsam::TONS_PER_BOX : tons));
}
...
void Ship::Jettison(const Outfit *outfit, int count) {
  ...
  const int perBox = (mass <= 0.)
                         ? count
                         : (mass > Flotsam::TONS_PER_BOX)
                               ? 1
                               : static_cast<int>(Flotsam::TONS_PER_BOX / mass);
  while (count > 0) {
    jettisoned.emplace_back(
        new Flotsam(outfit, (perBox < count) ? perBox : count));
    count -= perBox;
  }
}
...
const map<const Outfit *, int> &Ship::Outfits() const { return outfits; }
...
const vector<Hardpoint> &Ship::Weapons() const { return armament.Get(); }
...
shared_ptr<Ship> Ship::GetTargetShip() const { return targetShip.lock(); }

shared_ptr<Ship> Ship::GetShipToAssist() const { return shipToAssist.lock(); }
...
shared_ptr<Ship> Ship::GetParent() const { return parent.lock(); }

const vector<weak_ptr<Ship>> &Ship::GetEscorts() const { return escorts; }
...
double Ship::BestFuel(const string &type, const string &subtype,
                      double defaultFuel) const {
  // Find the outfit that provides the least costly hyperjump.
  double best = 0.;
  // Make it possible for a hyperdrive to be integrated into a ship.
  if (baseAttributes.Get(type) &&
      (subtype.empty() || baseAttributes.Get(subtype))) {
...
void Ship::CreateExplosion(vector<Visual> &visuals, bool spread) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (GetMask().Contains(point, Angle())) {
      ...
      visuals.emplace_back(*it->first, angle.Rotate(point) + position,
                           effectVelocity, angle);
      ++explosionCount;
      return;
    }
  }
...
void Ship::CreateSparks(vector<Visual> &visuals, const string &name,
                        double amount) {
  if (forget)
    return;
  ...
  while (true) {
    ...
    Point point((Random::Real() - .5) * Width(),
                (Random::Real() - .5) * Height());
    if (GetMask().Contains(point, Angle()))
      visuals.emplace_back(*effect, angle.Rotate(point) + position, velocity,
                           angle);
  }
}
