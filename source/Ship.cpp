...
double Ship::MaxVelocity() const
{
	...
	// v = thrust / drag
	double thrust = attributes.Get({THRUSTING, THRUST});
	return (thrust ? thrust : attributes.Get({AFTERBURNING, THRUST})) / Drag();
}
...
void Ship::DoGeneration()
{
	...
	if(!isDisabled)
	{
		...
		const double hullHeat = (attributes.Get({HULL_REPAIR, HEAT})
			* (1. + attributes.Get(AttributeAccess(HULL_REPAIR, HEAT)))) / hullAvailable;
...
void Ship::DoCloakDecision()
{
	...
	double cloakingSpeed = attributes.Get({CLOAKING, CLOAK});
