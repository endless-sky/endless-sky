...
namespace {
	...
	void AddController(int idx)
	{
		...
		Logger::Log(std::string("Added controller ") + SDL_GameControllerName(g_gc), Logger::Level::INFO);
	}

	// Remove a controller
	void RemoveController()
	{
		Logger::Log(std::string("Removing controller ") + SDL_GameControllerName(g_gc), Logger::Level::INFO);
...
void GamePad::SaveMapping()
{
	if(g_gc)
	{
		...
		std::shared_ptr<SDL_RWops> in(
			SDL_RWFromFile(MAPPING_FILE_PATH.c_str(), "rb"),
			[](SDL_RWops *p) {
				if(p) SDL_RWclose(p);
			}
		);
		...
		std::shared_ptr<SDL_RWops> out(
			SDL_RWFromFile(MAPPING_FILE_PATH.c_str(), "wb"),
			[](SDL_RWops *p) {
				if(p) SDL_RWclose(p);
			}
		);
...
void GamePad::Handle(const SDL_Event &event)
{
	...
	case SDL_JOYAXISMOTION:
		...
		// 						  " " + std::to_string(event.jaxis.value));
	...
	case SDL_JOYHATMOTION:
		// AddEventDebugString("Hat " + std::to_string(static_cast<int>(event.jhat.hat)) +
		//                     " mask " + std::to_string(static_cast<int>(event.jhat.value)));
...
void GamePad::SetControllerIdx(int idx)
{
	...
	{
		Logger::Log("Attempted to select an invalid controller " + std::to_string(idx), Logger::Level::WARNING);
	}
}
