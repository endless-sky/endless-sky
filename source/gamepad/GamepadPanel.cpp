	...
	ui(GameData::Interfaces().Get("gamepad panel"))
{
	...
	gamepadList->SetCallback([this](int idx, const std::string&) {
	...
	triggerThresholdList->SetSelected(
		std::to_string(
			static_cast<int>(GamePad::AxisIsButtonPressThreshold() * 100.0 / 32767 + .5)
		) + " %"
	);
...
void GamepadPanel::Draw()
{
	...
	if(remapIdx == -1) // not remapping. Just display the pressed buttons
	{
		if(buttons[SDL_CONTROLLER_BUTTON_B])
			info.SetBar("B Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_A])
			info.SetBar("A Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_Y])
			info.SetBar("Y Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_X])
			info.SetBar("X Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_GUIDE])
			info.SetBar("Guide Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_START])
			info.SetBar("Start Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_BACK])
			info.SetBar("Back Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_LEFTSHOULDER])
			info.SetCondition("Left Shoulder Button");
		if(buttons[SDL_CONTROLLER_BUTTON_RIGHTSHOULDER])
			info.SetCondition("Right Shoulder Button");
		if(axes[SDL_CONTROLLER_AXIS_TRIGGERLEFT] > 16000)
			info.SetCondition("Left Trigger Button");
		if(axes[SDL_CONTROLLER_AXIS_TRIGGERRIGHT] > 16000)
			info.SetCondition("Right Trigger Button");
		if(buttons[SDL_CONTROLLER_BUTTON_LEFTSTICK])
			info.SetBar("Left Stick Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_RIGHTSTICK])
			info.SetBar("Right Stick Button", 1);
		if(buttons[SDL_CONTROLLER_BUTTON_DPAD_LEFT])
			info.SetCondition("Left Dpad Button");
		if(buttons[SDL_CONTROLLER_BUTTON_DPAD_RIGHT])
			info.SetCondition("Right Dpad Button");
		...
		if(axes[SDL_CONTROLLER_AXIS_LEFTX] > 16000)
			info.SetCondition("Left Joystick Right");
		if(axes[SDL_CONTROLLER_AXIS_LEFTY] < -16000)
			info.SetCondition("Left Joystick Up");
		if(axes[SDL_CONTROLLER_AXIS_LEFTY] > 16000)
			info.SetCondition("Left Joystick Down");
		if(axes[SDL_CONTROLLER_AXIS_RIGHTX] < -16000)
			info.SetCondition("Right Joystick Left");
		if(axes[SDL_CONTROLLER_AXIS_RIGHTX] > 16000)
			info.SetCondition("Right Joystick Right");
		if(axes[SDL_CONTROLLER_AXIS_RIGHTY] < -16000)
			info.SetCondition("Right Joystick Up");
		if(axes[SDL_CONTROLLER_AXIS_RIGHTY] > 16000)
			info.SetCondition("Right Joystick Down");
	}
	else
	{
		...
		else if(static_cast<size_t>(remapIdx) < CONFIGURABLE_BUTTONS.size() + CONFIGURABLE_AXES.size())
		{
			...
			if(axis == SDL_CONTROLLER_AXIS_LEFTX)
				info.SetCondition(negative ? "Left Joystick Left" : "Left Joystick Right");
			if(axis == SDL_CONTROLLER_AXIS_LEFTY)
				info.SetCondition(negative ? "Left Joystick Up" : "Left Joystick Down");
			if(axis == SDL_CONTROLLER_AXIS_RIGHTX)
				info.SetCondition(negative ? "Right Joystick Left" : "Right Joystick Right");
			if(axis == SDL_CONTROLLER_AXIS_RIGHTY)
				info.SetCondition(negative ? "Right Joystick Up" : "Right Joystick Down");
		}
	}
