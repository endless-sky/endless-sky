...
bool MapDetailPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	else if((key == SDLK_TAB || command.Has(Command::JUMP)) && player.Flagship())
	{
		...
		// If a system is selected that is not located within the current travel plan, then the player selected it
		...
		if(selectedSystem != player.GetSystem() && find(plan.begin(), plan.end(), selectedSystem) == plan.end())
		{
