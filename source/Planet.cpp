...
namespace {
	...
	void SetRequiredAttributes(const set<string> &attributes, set<string> &required)
	{
		...
		required.clear();
		for_each(attributes.lower_bound(PREFIX), attributes.lower_bound(PREFIX_END),
...
void Planet::Load(const DataNode &node, Set<Wormhole> &wormholes)
{
			...
			for(const DataNode &grand : child)
			{
				...
				else if(grand.Token(0) == "fleet")
				{
					if(grand.Size() >= 2 && !grand.HasChildren())
					{
						...
						defenseFleets.insert(defenseFleets.end(), grand.Size() >= 3 ? grand.Value(2) : 1,
	...
	inhabited =
		(HasServices() || requiredReputation || !defenseFleets.empty()) && !attributes.count("uninhabited");
...
void Planet::FinishLoading(Set<Wormhole> &wormholes)
{
	...
	if(!wormhole && systems.size() > 1)
	{
		...
		Logger::LogError("Warning: deprecated automatic generation of wormhole \"" + name
						 + "\" from a multi-system planet.");
	}
...
bool Planet::IsValid() const
{
	return isDefined && !systems.empty()
		   && all_of(systems.begin(), systems.end(),
...
bool Planet::IsAccessible(const Ship *ship) const
{
	...
	if(wormhole && ship && ship->GetSystem()
		&& wormhole->WormholeDestination(*ship->GetSystem()).Inaccessible())
		return false;
	...
	return all_of(requiredAttributes.cbegin(), requiredAttributes.cend(),
		[&](const string &attr) -> bool
		{
			return shipAttributes.Get(attr);
		});
}
