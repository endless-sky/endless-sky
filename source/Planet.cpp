...
void Planet::Load(const DataNode &node)
{
	...
	// Update wormhole if it was autogenerated (for backwards compatibility).
	if(wormhole && wormhole->IsAutogenerated())
		wormhole->SetLinked(description.empty());
...
void Planet::SetSystem(const System *system)
{
	if(find(systems.begin(), systems.end(), system) == systems.end())
	{
		...
		if(!wormhole && systems.size() > 1 && IsValid())
			GameData::AutogenerateWormhole(this);
		// If the wormhole was autogenerated we need to update its links
		// every time the corresponding planet changes systems.
		if(wormhole && wormhole->IsAutogenerated())
			wormhole->GenerateLinks();
	}
}
