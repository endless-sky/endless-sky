...
public:
	...
	// sorted by distance.
	std::vector<Collision> &Line(const Projectile &projectile, std::vector<Collision> &result) const;

	// Get all possible collisions along a line. Collisions are not necessarily sorted by
	// distance.
	std::vector<Collision> &Line(const Point &from, const Point &to, std::vector<Collision> &result,
		const Government *pGov = nullptr, const Body *target = nullptr) const;

	// Get all objects within the given range of the given point.
	std::vector<Body *> &Circle(const Point &center, double radius, std::vector<Body *> &result) const;
	// Get all objects touching a ring with a given inner and outer range
	// centered at the given point.
	std::vector<Body *> &Ring(const Point &center, double inner, double outer, std::vector<Body *> &result) const;
