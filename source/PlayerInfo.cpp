...
void PlayerInfo::Land(UI *ui)
{
  ...
  bool canUseServices = planet->CanUseServices();
	bool hasSpaceport = planet->HasSpaceport() && canUseServices;
  bool hasOutfitter = planet->HasOutfitter() && canUseServices;
  ...
  // Alert when there's spaceport missions available
  if (Preferences::Has("Spaceport mission alert") && hasSpaceport && MissionToOffer(Mission::SPACEPORT))
  {
    Messages::Add("There is a spaceport mission available.");
    ui->Push(new Dialog("There is a spaceport mission available."));
  }

  // Alert when you can buy a map
  if (Preferences::Has("Local map available alert") && hasOutfitter)
  {
    // Hard coded distance, this is ugly.
    DistanceMap distance(GetSystem(), 12);
    for(const System *system : distance.Systems())
    {
      if(!HasVisited(system))
      {
        Messages::Add("There is new local map information available in outfitting.");
        ui->Push(new Dialog("There is new local map information available in outfitting."));
        break;
      }
    }
  }
