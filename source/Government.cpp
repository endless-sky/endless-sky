...
// Determine the penalty for the given ShipEvent based on the values in the
// given map.
...
void Government::Load(const DataNode &node,
    ...
    for (const DataNode &child : node) {
        ...
        if ((add || remove) && child.Size() < 2) {
            child.PrintTrace("Skipping " + child.Token(0) +
                             " with no key given:");
            continue;
        }
        ...
        // Handle the attributes which cannot have a value removed.
            ...
            for (const DataNode &grand : child) {
                ...
                if (grandKey == "player reputation" && hasGrandValue)
                    initialPlayerReputation =
                        add ? initialPlayerReputation + grand.Value(valueIndex)
                            : grand.Value(valueIndex);
                else if (grandKey == "max" && hasGrandValue)
                    reputationMax =
                        add ? reputationMax + grand.Value(valueIndex)
                            : grand.Value(valueIndex);
                else if (grandKey == "min" && hasGrandValue)
                    reputationMin =
                        add ? reputationMin + grand.Value(valueIndex)
                            : grand.Value(valueIndex);
                else
                    grand.PrintTrace("Skipping unrecognized attribute:");
            }
            ...
            for (const DataNode &grand : child) {
                ...
                if ((add || remove) && grand.Size() < 2) {
                    grand.PrintTrace("Warning: Skipping invalid \"" +
                                     child.Token(0) + "\" tag:");
                    continue;
                }
                ...
                const Government *gov =
                    GameData::Governments().Get(grand.Token(remove || add));
                if (gov) {
                    if (remove)
                        trusted.erase(gov);
                    else
            ...
            for (const DataNode &grand : child) {
                ...
                if (grandKey == "remove" && grand.Size() >= 2)
                    customPenalties
                        [GameData::Governments().Get(grand.Token(1))->id]
                            .clear();
                else {
                    auto &pens = customPenalties
                        [GameData::Governments().Get(grandKey)->id];
                ...
                if (grandKey == "remove") {
                    if (grand.Token(1) == "ship" && grand.Size() >= 3) {
                        if (!illegalShips.erase(grand.Token(2)))
                            grand.PrintTrace("Invalid remove, ship not found "
                                             "in existing illegals:");
                    } else if (!illegalOutfits.erase(
                                   GameData::Outfits().Get(grand.Token(1))))
                        grand.PrintTrace("Invalid remove, outfit not found in "
                                         "existing illegals:");
                } else if (grandKey == "ignore") {
                    if (grand.Token(1) == "ship" && grand.Size() >= 3)
                        illegalShips[grand.Token(2)] = 0;
                    else
                        illegalOutfits[GameData::Outfits().Get(
                            grand.Token(1))] = 0;
                } else if (grandKey == "ship" && grand.Size() >= 3)
                    illegalShips[grand.Token(1)] = grand.Value(2);
                else
                    illegalOutfits[GameData::Outfits().Get(grandKey)] =
                        grand.Value(1);
            }
        } else if (key == "atrocities") {
            if (!add) {
            ...
            if (add)
                ...
                travelRestrictions =
                    LocationFilter(child, visitedSystems, visitedPlanets);
        } else if (key == "foreign penalties for")
            for (const DataNode &grand : child)
                useForeignPenaltiesFor.insert(
                    GameData::Governments().Get(grand.Token(0))->id);
        else if (key == "send untranslated hails")
            ...
            initialPlayerReputation =
                add ? initialPlayerReputation + child.Value(valueIndex)
                    : child.Value(valueIndex);
        else if (key == "crew attack")
            crewAttack = max(0., add ? child.Value(valueIndex) + crewAttack
                                     : child.Value(valueIndex));
        else if (key == "crew defense")
            crewDefense = max(0., add ? child.Value(valueIndex) + crewDefense
                                      : child.Value(valueIndex));
        else if (key == "bribe")
            bribe =
                add ? bribe + child.Value(valueIndex) : child.Value(valueIndex);
        else if (key == "fine")
            fine =
                add ? fine + child.Value(valueIndex) : child.Value(valueIndex);
        else if (add)
            ...
            if (child.Size() >= 3 + valueIndex)
                color = ExclusiveItem<Color>(
                    Color(child.Value(valueIndex), child.Value(valueIndex + 1),
                          child.Value(valueIndex + 2)));
            else if (child.Size() >= 1 + valueIndex)
                color = ExclusiveItem<Color>(
                    GameData::Colors().Get(child.Token(valueIndex)));
        } else if (key == "death sentence")
            deathSentence =
                GameData::Conversations().Get(child.Token(valueIndex));
        else if (key == "friendly hail")
            friendlyHail = GameData::Phrases().Get(child.Token(valueIndex));
        else if (key == "friendly disabled hail")
            friendlyDisabledHail =
                GameData::Phrases().Get(child.Token(valueIndex));
        else if (key == "hostile hail")
            ...
            enforcementZones.clear();
            child.PrintTrace("Warning: Deprecated use of \"enforces all\". Use "
                             "\"remove enforces\" instead:");
        } else
            child.PrintTrace("Skipping unrecognized attribute:");
    }
...
const string &Government::GetName() const { return displayName; }

// Set / Get the name used for this government in the data files.
void Government::SetName(const string &trueName) { this->name = trueName; }

const string &Government::GetTrueName() const { return name; }
...
// Get the amount that your reputation changes for the given offense against the
// given government. The given value should be a combination of one or more
// ShipEvent values. Returns 0 if the Government is null.
double Government::PenaltyFor(int eventType, const Government *other) const {
    ...
    const auto &penalties =
        useForeignPenaltiesFor.contains(id) ? other->penaltyFor : penaltyFor;
...
string Government::GetBribeAcceptanceHail() const {
    return bribeAcceptanceHail ? bribeAcceptanceHail->Get()
                               : "It's a pleasure doing business with you.";
}

string Government::GetBribeRejectionHail() const {
    return bribeRejectionHail ? bribeRejectionHail->Get()
                              : "I do not want your money.";
}

// Find out if this government speaks a different language.
const string &Government::Language() const { return language; }

// Find out if this government should send custom hails even if the player does
// not know its language.
...
// The second attribute denotes the minimal and maximal attraction required for
// the fleet to appear.
const vector<RaidFleet> &Government::RaidFleets() const { return raidFleets; }
...
string Government::Fine(PlayerInfo &player, int scan, const Ship *target,
...
bool Government::Condemns(const Outfit *outfit) const {
    ...
    return (found && isAtrocity->second) ||
           (!found && outfit->Get("atrocity") > 0.);
}
...
bool Government::Condemns(const Ship *ship) const {
    ...
    return (found && isAtrocity->second) ||
           (!found && ship->BaseAttributes().Get("atrocity") > 0.);
}
