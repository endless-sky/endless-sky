...
Preset::Preset(string name, string shipModel, const map<const Outfit*, int> &outfits)
    : name(std::move(name)),
...
Preset* Preset::Load(const filesystem::path &path)
{
	...
	// If the named ship model exists and there's a nonzero number of outfits, or the preset is marked as empty, we have a valid preset.
