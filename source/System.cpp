...
namespace {
	...
	double SolarGenerationScale(const Point &starPosition, const Point &shipPosition)
	{
		return .2 + 1.8 / (.001 * starPosition.Distance(shipPosition) + 1);
	}
}
...
// Return how much solar energy/heat is generated by this system
// for a ship with the given solar collection/heat.
double System::SolarPower(double shipCollection, const Point &shipPosition) const
{
	double total = 0.;
	for(const auto &stellar : objects)
	{
		double solarPower = GameData::SolarPower(stellar.GetSprite());
		if(!solarPower)
			continue;
		total += SolarGenerationScale(stellar.Position(), shipPosition) * shipCollection * solarPower;
	}
	return total;
}
...
// for a ship with the given ramscoop value.
double System::RamscoopFuel(double shipRamscoop, const Point &shipPosition) const
{
	double total = ramscoopAddend;
	for(const auto &stellar : objects)
	{
		double solarWind = GameData::SolarWind(stellar.GetSprite());
		if(!solarWind)
			continue;
		double scale = SolarGenerationScale(stellar.Position(), shipPosition);
		...
		total += solarWind * .03 * scale * ramscoopMultiplier * (sqrt(shipRamscoop) + universal);
	}
	return max(0., total);
}
