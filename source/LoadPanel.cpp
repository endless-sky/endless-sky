...
void LoadPanel::Draw()
{
	...
	for(const auto &it : files)
	{
		...
		if(it.first == selectedPilot)
			FillShader::UISpace::Fill(zone.Center(), zone.Dimensions(), Color(.1 * alpha, 0.));
	...
	if(!hoverText.empty())
	{
		...
		FillShader::UISpace::Fill(hoverPoint + .5 * boxSize, boxSize, *GameData::Colors().Get("tooltip background"));
