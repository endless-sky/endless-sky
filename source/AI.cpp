	...
	const StellarObject *GetRefuelLocation(const Ship &ship)
	{
		...
		if(system)
		{
			...
			for(const StellarObject &object : system->Objects())
				if(object.HasSprite() && object.GetPlanet() && object.GetPlanet()->IsValid()
						&& !object.GetPlanet()->IsWormhole() && object.GetPlanet()->HasFuelFor(ship))
				{
	...
	void SelectRoute(Ship &ship, const System *targetSystem)
	{
		...
		if(to && !needsRefuel)
			for(const StellarObject &object : from->Objects())
			{
				...
				if(object.HasSprite() && planet && planet->IsValid() && planet->IsWormhole()
						&& planet->IsAccessible(&ship) && planet->WormholeDestination(from) == to)
				{
...
shared_ptr<Ship> AI::FindTarget(const Ship &ship) const
{
				...
				&& !person.IsStaying() && !parentIsEnemy)))
	{
		...
		if(ship.JumpsRemaining() && (!system->Links().empty() || ship.Attributes().Get("jump drive")))
			...
			for(const StellarObject &object : system->Objects())
				if(object.HasSprite() && object.GetPlanet() && object.GetPlanet()->IsValid()
						&& object.GetPlanet()->HasSpaceport() && object.GetPlanet()->CanLand(ship))
				{
					target.reset();
					break;
				}
...
void AI::MovePlayer(Ship &ship, const PlayerInfo &player, Command &activeCommands)
{
	...
	else if(activeCommands.Has(Command::LAND) && !ship.IsEnteringHyperspace() && ship.Zoom() == 1.)
	{
		...
		for(const StellarObject &object : ship.GetSystem()->Objects())
		{
			if(object.HasSprite() && object.GetPlanet() && object.GetPlanet()->IsValid() && object.GetPlanet()->IsAccessible(&ship))
