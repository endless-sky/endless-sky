...
{
	std::string ret;
	if (state & MENU.state) ret += " MENU";
	if (state & FORWARD.state) ret += " FORWARD";
	if (state & LEFT.state) ret += " LEFT";
	if (state & RIGHT.state) ret += " RIGHT";
	if (state & BACK.state) ret += " BACK";
	if (state & PRIMARY.state) ret += " PRIMARY";
	if (state & SECONDARY.state) ret += " SECONDARY";
	if (state & SELECT.state) ret += " SELECT";
	if (state & LAND.state) ret += " LAND";
	if (state & BOARD.state) ret += " BOARD";
	if (state & HAIL.state) ret += " HAIL";
	if (state & SCAN.state) ret += " SCAN";
	if (state & JUMP.state) ret += " JUMP";
	if (state & FLEET_JUMP.state) ret += " FLEET_JUMP";
	if (state & TARGET.state) ret += " TARGET";
	if (state & NEAREST.state) ret += " NEAREST";
	if (state & DEPLOY.state) ret += " DEPLOY";
	if (state & AFTERBURNER.state) ret += " AFTERBURNER";
	if (state & CLOAK.state) ret += " CLOAK";
	if (state & MAP.state) ret += " MAP";
	if (state & INFO.state) ret += " INFO";
	if (state & FULLSCREEN.state) ret += " FULLSCREEN";
	if (state & FASTFORWARD.state) ret += " FASTFORWARD";
	if (state & FIGHT.state) ret += " FIGHT";
	if (state & GATHER.state) ret += " GATHER";
	if (state & HOLD.state) ret += " HOLD";
	if (state & AMMO.state) ret += " AMMO";
	if (state & WAIT.state) ret += " WAIT";
	if (state & STOP.state) ret += " STOP";
	if (state & SHIFT.state) ret += " SHIFT";
	if (state & MOUSE_TURNING_HOLD.state) ret += " MOUSE_TURNING_HOLD";
	if (state & MOUSE_TURNING_TOGGLE.state) ret += " MOUSE_TURNING_TOGGLE";
	if (state & MOVETOWARD.state) ret += " MOVETOWARD";
	return ret;
}