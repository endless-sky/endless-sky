...
void Music::SetSource(const string &name)
{
	...
	// Wait for the running decoding task to finish.
	if(decoder.valid())
		decoder.wait();

	// Switch decoding to the new file.
	file = path.empty() ? nullptr : Files::Open(path);

	// Also clear any decoded data left over from the previous file.
	next.clear();

	// Now that we have a file to read initialize the decoder.
	mad_stream_init(&stream);
	mad_frame_init(&frame);
	mad_synth_init(&synth);

	// Start decoding a single frame.
	DecodeFrame();
}
...
const Music::Buffer &Music::NextChunk()
{
	// Check whether the "next" buffer is ready.
	if(!decoder.valid() || decoder.wait_for(chrono::seconds(0)) != future_status::ready)
		return silence;
