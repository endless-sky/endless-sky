...
HailPanel::HailPanel(PlayerInfo &player, const shared_ptr<Ship> &ship,
    ...
    bool isMute = ship->GetPersonality().IsMute() ||
                  (ship->Attributes().Category() == "Drone");
    hasLanguage =
        !isMute && (gov->Language().empty() ||
    ...
    if (isMute)
        ...
        if (flagship->NeedsFuel(false) || flagship->IsDisabled())
            SetMessage(
                "Sorry, we can't help you, because our ship is disabled.");
    } else {
        ...
        if (ship->GetShipToAssist() == player.FlagshipPtr())
            ...
            string helpOffer =
                "Looks like you've gotten yourself into a bit of trouble. "
            ...
            SetMessage("Sorry, my ship is too small to have the right "
...
bool HailPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command,
    ...
    const Government *gov = ship ? ship->GetGovernment()
                                 : (planet ? planet->GetGovernment() : nullptr);
    ...
    if (key == 'd' || key == SDLK_ESCAPE || key == SDLK_RETURN ||
        ...
        if (GameData::GetPolitics().HasDominated(planet)) {
            ...
            GetUI()->Push(new Dialog(
                [this]() { SetMessage(planet->DemandTribute(player)); },
                "Demanding tribute may cause this planet to launch defense "
                "fleets to fight you. "
                "After battling the fleets, you can demand tribute again for "
                "the planet to relent.\n"
                "This act may hurt your reputation severely. Do you want to "
                "proceed?",
        ...
        if (playerNeedsHelp) {
            if (ship->GetPersonality().IsSurveillance())
                ...
                if (ship->Fuel())
                    SetMessage("Sorry, but if we give you fuel we won't have "
                               "enough to make it to the next system.");
                else
                    SetMessage("Sorry, we don't have any fuel.");
            } else if (player.Flagship()->NeedsEnergy()) {
                if (ship->Energy())
                    SetMessage("Sorry, but if we give you energy we won't have "
                               "enough for our ship.");
                else
            ...
            if (bribe)
                ...
                SetMessage("You don't seem to be in need of repairs or fuel "
                           "assistance.");
        }
        ...
        if (bribe > player.Accounts().Credits())
            SetMessage(
                "Sorry, but you don't have enough money to be worth my while.");
        else if (bribe) {
            ...
            if (ship) {
                if (!requestedToBribeShip) {
                    SetMessage(
                        "If you want us to leave you alone, it'll cost you " +
                        Format::CreditString(bribe) + ".");
                    requestedToBribeShip = true;
                } else {
                    bribed = ship->GetGovernment();
                    bribed->Bribe();
                    Messages::Add("You bribed a " + bribed->GetName() +
                                      " ship " + Format::CreditString(bribe) +
                                      " to refrain from attacking you today.",
                                  Messages::Importance::High);
                }
            } else {
                planet->Bribe();
                Messages::Add("You bribed the authorities on " +
                                  planet->DisplayName() + " " +
                                  Format::CreditString(bribe) +
                                  " to permit you to land.",
                              Messages::Importance::High);
            }
        } else
