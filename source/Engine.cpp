...
void Engine::CalculateStep()
{
	...
	for_each(std::execution::par, ships.begin(), ships.end(), [&](const shared_ptr<Ship> &it)
	{
	...
	for_each(std::execution::par, projectiles.begin(), projectiles.end(), [&](auto &projectile) {
...
void Engine::DoCollisions(list<Visual> &visuals, Projectile &projectile)
{
	...
	sort(std::execution::par_unseq, collisions.begin(), collisions.end());
