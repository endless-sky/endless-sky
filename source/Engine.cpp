...
void Engine::Step(bool isActive)
{
	...
	int recentChange = player.Accounts().RecentChange();
	if(recentChange)
	{
		int framesRemaining = player.Accounts().Actualize();
		if(framesRemaining > 120)
			info.SetString("recent change", Format::Credits(recentChange));
		else if(framesRemaining > 70)
			info.SetString("recent change medium", Format::Credits(recentChange));
		else if(framesRemaining > 0)
			info.SetString("recent change faint", Format::Credits(recentChange));
	}
