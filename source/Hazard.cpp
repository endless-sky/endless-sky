...
void Hazard::Load(const DataNode &node)
{
	...
	for(const DataNode &child : node)
	{
		...
		else if(key == "length")
		{
			...
			maxLength = max(0, (child.Size() > 2 ? static_cast<int>(child.Value(2)) : 0));
		}
		else if (key == "strength")
		{
			minStrength = max(0., child.Value(1));
			maxStrength = max(0., (child.Size() > 2) ? child.Value(2) : 0.);
		}
		else if(key == "range")
			range = max(0., child.Value(1));
