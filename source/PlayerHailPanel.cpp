...
PlayerHailPanel::PlayerHailPanel(PlayerInfo &player, const StellarObject *object)
...
void PlayerHailPanel::Draw()
{
	DrawHail();
	ship ? DrawIcon(*ship) : DrawIcon(*sprite, facing);
	...
	const Interface *hailUi = GameData::Interfaces().Get("player hail panel");
	hailUi->Draw(info, this);
}



bool PlayerHailPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
...
void PlayerHailPanel::SetBribe(double scale)
{
