...
void ConditionSet::Add(const DataNode &node)
{
	...
	for(const string &token : node.Tokens())
		if(DataNode::IsCondition(token))
			node.PrintTrace(string("Warning: Only condition references should begin with ")
				+ "\"" + DataNode::CONDITION_CHAR + "\"."
				" Unexpected \"" + DataNode::CONDITION_CHAR + "\" in (" + token + ").");
