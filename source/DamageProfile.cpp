...
void DamageProfile::PopulateDamage(DamageDealt &damage, const Ship &ship) const
{
	...
	if(shields > 0.)
	{
		...
		if(highPermeability || lowPermeability)
		{
			...
			permeability =
				max((highPermeability * shieldPortion) + (lowPermeability * (1. - shieldPortion)), 0.);
		}
	...
	double totalHullProtection = (ScaleType(1., 0.,
		attributes.Get("hull protection")
			+ (ship.IsCloaked() ? attributes.Get("cloak hull protection") : 0.)));
	damage.hullDamage =
		(weapon.HullDamage() + weapon.RelativeHullDamage() * ship.MaxHull()) * totalHullProtection;
	...
	damage.dischargeDamage =
		weapon.DischargeDamage() * ScaleType(0., .5, attributes.Get("discharge protection"));
	damage.corrosionDamage =
		weapon.CorrosionDamage() * ScaleType(1., 0., attributes.Get("corrosion protection"));
	...
	damage.scramblingDamage =
		weapon.ScramblingDamage() * ScaleType(.5, 0., attributes.Get("scramble protection"));
	damage.disruptionDamage =
		weapon.DisruptionDamage() * ScaleType(0., .5, attributes.Get("disruption protection"));
