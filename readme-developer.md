...
Two methods of building the game on Windows are available: via the Code::Blocks IDE, and via Scons on a command line. Both options require a MinGW toolchain, and precompiled libraries provided below.
...
Endless Sky will also require precompiled libraries to build and run. Download them [here](http://endless-sky.github.io/win64-dev.zip), extract the zip, then move the `dev64` folder to `C:\` in order to build the game without further configuration.


### Building with CodeBlocks

If you want to build the game with a visual IDE, then you can use [CodeBlocks](https://codeblocks.org/downloads/26), the main supported IDE for Endless Sky on Windows. As you have already installed a compiler via MinGW, you should select a version without one.

After you install it, CodeBlocks will likely detect a "GNU GCC Compiler" from MinGW. Select this as your compiler, then take these steps to configure it:
...
If you instead want to build the game from the command line via Scons, you will need [Python 3.8 or later](https://www.python.org/downloads/). When installing, be sure to select the "Add to PATH" checkbox.
...
From there, restart your command line if you had one open, and simply `cd` to your Endless Sky directory, then type `mingw32-make.exe -f .winmake`. To speed up the build process, you can add a `-jX` flag to that command, where X is the number of CORES/THREADS? your processor has.

### Building with a third-party IDE
...
- If you built the game via CodeBlocks, double click *EndlessSky* in the file panel on the left, then either click the green arrow on top, or hit Control+F10.
